...
RunRF <- function(Train_set, Train_label, mode, classVar){
  ...
  original_names <- colnames(Train_set)
  legal_names    <- make.names(original_names)
  colnames(Train_set) <- legal_names
  df <- as.data.frame(cbind(Train_set, Train_label[classVar]))
  fit <- rfsrc(formula = formula(paste0(classVar, "~.")), 
               data = df, 
               ntree = 1000, nodesize = rf_nodesize,
               importance = TRUE, proximity = TRUE, forest = TRUE)
  fit$subFeature <- original_names
  selected_vars <- ExtractVar(fit)
  fit$subFeature <- selected_vars
  if (mode == "Model") {
    df_selected <- as.data.frame(cbind(Train_set[, selected_vars], Train_label[classVar]))
    colnames(df_selected) <- c(make.names(selected_vars), classVar)
    fit_final <- rfsrc(formula = formula(paste0(classVar, "~.")), 
                       data = df_selected, 
                       ntree = 1000, nodesize = rf_nodesize,
                       importance = TRUE, proximity = TRUE, forest = TRUE)
    fit_final$subFeature <- selected_vars
    return(fit_final)
  }
  if (mode == "Variable") return(selected_vars)
}
...
ExtractVar <- function(fit){
  ...
  imp <- imp[seq_along(fit$subFeature)]
  top_n <- 5
  idx <- order(imp, decreasing = TRUE)[1:min(top_n, length(imp))]
  fit$subFeature[idx]
},
...
RunEval <- function(fit,
  ...
  if ((!is.null(Train_set)) & (!is.null(Train_label))) {
    ...
    if (!is.null(Train_name)) {
      ...
      Train_label[[cohortVar]] <- "Training"
    }
  ...
  Predict.out <- data.frame(Test_label, RS = as.vector(RS))
  Predict.out <- Predict.out[!is.na(Predict.out[[classVar]]), ]
  Predict.out <- Predict.out[complete.cases(Predict.out$RS), ]
  Predict.out <- split(Predict.out, Predict.out[, cohortVar])
  auc_list <- lapply(Predict.out, function(data) {
    resp <- data[[classVar]]
    if (length(unique(resp)) < 2) {
      warning(sprintf("Cohort [%s] has < 2 classes, skip AUC calculation.",
                      unique(data[[cohortVar]])))
      return(NA_real_)
    }
    auc_obj <- suppressMessages(roc(resp, data$RS))
    as.numeric(auc(auc_obj))
  })
  unlist(auc_list)
}
